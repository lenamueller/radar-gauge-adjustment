...
from func import get_depths, max_from_arrays, plot_grid
...
radar = domain
np.savetxt("geodata/radar.txt", radar, fmt = "%.4f")

# Plot composite.
plot_grid(radar, xgrid, ygrid, plottitle='09-01-2019 12:00 UTC\nDWD RADAR composite\nUTM zone 33N (EPSG 32633)', 
          filename=f"composite_{filename_drs[15:25]}_utm60min")
...
pl.savefig(f"images/gaugedata_1901091200_60min", dpi=600)
...
fig = pl.figure(figsize=(10,8))
ax = pl.subplot(111, aspect="equal")
pm = pl.pcolormesh(xgrid, ygrid, gauge_array_int, cmap=cm, vmin=0, vmax=max(gaugedata["prec_mm"]))
cbar = pl.colorbar(pm)
cm.set_bad(color='grey')
cbar.set_label("60 min - rain depths (mm)", fontsize=12)
pl.xlabel("Easting (m)")
...
pl.grid(lw=0.5)
pl.title('09-01-2019 12:00 UTC\nDWD gauge data\nUTM zone 33N (EPSG 32633)', fontsize=12)
pl.savefig(f"images/gaugedata_1901091200_60min_interpol", dpi=600)
...
addadjuster = wrl.adjust.AdjustAdd(obs_coords, radar_coords, nnear_raws=3, minval=0)
addadjusted = addadjuster(obs, radar_1d)
addadjusted_arr = addadjusted.reshape(gridshape)

multadjuster = wrl.adjust.AdjustMultiply(obs_coords, radar_coords, nnear_raws=3, minval=0.01)
multadjusted = multadjuster(obs, radar_1d) 
multadjusted_arr = multadjusted.reshape(gridshape)

mfbadjuster = wrl.adjust.AdjustMFB(obs_coords, radar_coords)
mfbadjusted = mfbadjuster(obs, radar_1d)
mfbadjusted_arr = mfbadjusted.reshape(gridshape)

mixadjuster = wrl.adjust.AdjustMixed(obs_coords, radar_coords, nnear_raws=3, minval=0)
mixadjusted = mixadjuster(obs, radar_1d)
mixadjusted_arr = mixadjusted.reshape(gridshape)

# Plot adjusted radar data.
plot_grid(addadjusted_arr, xgrid, ygrid, "Additive adjustment (spatially variable)", "adjustment_add")
plot_grid(multadjusted_arr, xgrid, ygrid,"Multiplicative adjustment (spatially variable)", "adjustment_mul")
plot_grid(mfbadjusted_arr, xgrid, ygrid,"Multiplicative adjustment (spatially uniform)", "adjustment_mfb")
plot_grid(mixadjusted_arr, xgrid, ygrid,"Additive-multiplicative-mixed adjustment (spatially variable)", "adjustment_mixed")

# Plot errors.
plot_grid(addadjusted_arr - radar, xgrid, ygrid,"Additive error (spatially variable)", "adjustment_add_diff")
plot_grid(multadjusted_arr - radar, xgrid, ygrid,"Multiplicative error (spatially variable)", "adjustment_mul_diff")
plot_grid(mfbadjusted_arr - radar, xgrid, ygrid,"adjustment_mfb_diff (spatially uniform)", "adjustment_mfb_diff")
plot_grid(mixadjusted_arr - radar, xgrid, ygrid,"Additive-multiplicative-mixed error (spatially variable)", "adjustment_mixed_diff")

# Cumulative distribution function of gauges and radar.
fig = pl.figure(figsize=(10, 6))
n_gauges = plt.hist(obs, 50, density=True, histtype="step", cumulative=True, label="gauges", log=False, linewidth=1.5)
n_radar = plt.hist(np.round(radar_1d, decimals=1), 50, density=True, histtype="step", cumulative=True, label="raw radar", log=False, linewidth=1.5)
...
plt.savefig("images/adjustment_eval1", dpi=600)

# Cumulative distribution function of gauges and adjustment methods.
fig = pl.figure(figsize=(10, 6))
...
plt.xlabel("Precipitation [mm]", fontsize=12)
plt.ylabel("CDF", fontsize=12)
plt.savefig("images/adjustment_eval2", dpi=600)

# Cumulative distribution function of errors.
fig = pl.figure(figsize=(10, 6))
plt.hist(np.round(addadjusted - radar_1d, decimals=1), 100, density=True, label="add error", log=False, linewidth=1.5)
plt.hist(np.round(multadjusted - radar_1d, decimals=1), 100, density=True, label="mul error", log=False, linewidth=1.5)
plt.hist(np.round(mixadjusted - radar_1d, decimals=1), 100, density=True, label="mix error", log=False, linewidth=1.5)
plt.legend(loc="lower right")
plt.grid()
plt.xlim([-2,2])
plt.xlabel("radar error [mm]", fontsize=12)
plt.ylabel("CDF", fontsize=12)
plt.savefig("images/adjustment_eval3", dpi=600)
