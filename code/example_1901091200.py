...
from func import get_depths, max_from_arrays
...
domain[:] = np.NaN
domain = blending_radar_domains(domain, gridded_drs)
...
pl.savefig(f"images/composite_{filename_drs[15:25]}_utm60min", dpi=600)
...
plt.figure()
binwidth=0.1
plt.hist(obs, bins=9, density=True, color="blue", alpha=0.5, label="observation")
plt.hist(radar_1d, bins=29, density=True, color="red", alpha=0.5, label="radar")
plt.legend()
plt.yscale("log")
plt.xlabel("precipitation depth bins")
