...
# Plot composite.
plot_grid(radar, xgrid, ygrid, 
          plottitle='09-01-2019 12:00 UTC\nDWD RADAR composite\nUTM zone 33N (EPSG 32633)', 
...
gauge_files = [f for f in os.listdir("opendata.dwd.de/example_data/hourly_data/") if os.path.isfile(os.path.join("opendata.dwd.de/example_data/hourly_data/", f))]
myProj = Proj("+proj=utm +zone=33 +north +ellps=WGS84 +datum=WGS84 +units=m +no_defs")

for file in gauge_files: # Go through data files and fill gauge dict with id and precipitation.
...
gaugedata["easting"] = [0 for _ in range(num_gauges)] # Add easting, northing and elevation for given id's.
...
gauge_array_ipol = sp.interpolate.griddata(points = gauge_indices, values = gaugedata["prec_mm"], xi = all_indices, method="cubic")
gauge_array_ipol = gauge_array_ipol.reshape((700,700))

# Plot gauges data.
plot_grid(gauge_array, xgrid, ygrid, plottitle="09-01-2019 12:00 UTC\nDWD gauge data\nUTM zone 33N (EPSG 32633)", 
          filename="gaugedata_1901091200_60min")
plot_grid(gauge_array_ipol, xgrid, ygrid, plottitle="09-01-2019 12:00 UTC\nDWD gauge data\nUTM zone 33N (EPSG 32633)", 
          filename="gaugedata_1901091200_60min_ipol")

# radar and gauge data
radar_1d = radar.reshape([700*700])
obs_1d = np.array(gaugedata["prec_mm"])

# radar and gauge location
...
addadjuster = wrl.adjust.AdjustAdd(obs_coords, radar_coords, nnear_raws=9, minval=0)
multadjuster = wrl.adjust.AdjustMultiply(obs_coords, radar_coords, nnear_raws=9, minval=0.01)
mfbadjuster = wrl.adjust.AdjustMFB(obs_coords, radar_coords)
mixadjuster = wrl.adjust.AdjustMixed(obs_coords, radar_coords, nnear_raws=9, minval=0)

addadjusted = addadjuster(obs_1d, radar_1d)
multadjusted = multadjuster(obs_1d, radar_1d) 
mfbadjusted = mfbadjuster(obs_1d, radar_1d)
mixadjusted = mixadjuster(obs_1d, radar_1d)
...
plot_grid(addadjusted_arr, xgrid, ygrid, "Additive adjustment\n(spatially variable)", "adjustment_add")
plot_grid(multadjusted_arr, xgrid, ygrid,"Multiplicative adjustment\n(spatially variable)", "adjustment_mul")
plot_grid(mfbadjusted_arr, xgrid, ygrid,"Multiplicative adjustment\n(spatially uniform)", "adjustment_mfb")
plot_grid(mixadjusted_arr, xgrid, ygrid,"Additive-multiplicative-mixed adjustment\n(spatially variable)", "adjustment_mixed")

# Plot errors.
plot_grid(addadjusted_arr - radar, xgrid, ygrid,"Additive error\n(spatially variable)", "adjustment_add_diff")
plot_grid(multadjusted_arr - radar, xgrid, ygrid,"Multiplicative error\n(spatially variable)", "adjustment_mul_diff")
plot_grid(mfbadjusted_arr - radar, xgrid, ygrid,"adjustment_mfb_diff\n(spatially uniform)", "adjustment_mfb_diff")
plot_grid(mixadjusted_arr - radar, xgrid, ygrid,"Additive-multiplicative-mixed error\n(spatially variable)", "adjustment_mixed_diff")
